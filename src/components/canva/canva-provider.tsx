...
export default function CanvaProvider({ children }: Props) {
    ...
    useEffect(() => {
        ...
        const color = getComputedStyle(document.documentElement).getPropertyValue("--color-canva-circle").trim();
        ...
        if(typeof window !== "undefined") {
            ...
            if(canvasRef && canvasRef.current) {
                ...
                class Circle {
                    ...
                    constructor(x: number, y: number, dy: number, dx: number, r: number, mxr: number) {
    ...
    return (
        <>
        <canvas className="-z-10 bg-background fixed top-0 left-0" ref={canvasRef}></canvas>
        {children}
        </>
    )
